...
def _fire_laser(laser, laser_pulse_sec: float, state_debug_log: bool) -> None:
    ...
    if state_debug_log:
        ...
        print(f"[LASER] FIRE burst {fire_sec:.2f}s", flush=True)
    
    laser.pulse_async(fire_sec)
...
def main(
    ...
    # model = YOLO(str(path))
    print("[STEP] Loading YOLO...")
    model = YOLO(str(path))
    print("[STEP] YOLO loaded")
    ...
    try:
        while True:
            ...
            if fixed_cam:
                ...
                if show:
                    ...
                    if zone_target is not None:
                        ...
                        cv2.putText(
                            frame,
                            f"fixed conf={zone_target['conf']:.2f} c={confirm_count}",
                            (int(x1), max(0, int(y1) - 8)),
                            cv2.FONT_HERSHEY_SIMPLEX,
                            0.5,
                            (0, 255, 255),
            ...
            # TASK: Multi-target state machine (SCAN/AIM/SETTLE/FIRE/COOLDOWN).
            if multi_target:
                now_ts = time.monotonic()
                ...
                if sm_state == "SCAN":
                    if dets:
                        ...
                        sm_target = None
                        if state_debug_log:
                            print(f"[SM] t={now_ts:.3f} SCAN->AIM targets={len(sm_queue)}")
                        sm_state = "AIM"
                ...
                if sm_state == "AIM":
                    if sm_target is None:
                        if sm_queue:
                            ...
                            if state_debug_log:
                                print(
                                    f"[SM] t={now_ts:.3f} AIM pick conf={sm_target['conf']:.2f} remaining={len(sm_queue)}"
                                )
                        else:
                    ...
                    if sm_target is not None:
                        ...
                        tilt = _clamp(tilt, 75, 105)
                        print(
                            f"[DEBUG] cx={sm_target['cx']:.0f} "
                            f"center={w/2:.0f} "
                            f"pan={pan:.1f} "
                            f"tilt={tilt:.1f}"
                        )
                ...
                if sm_state == "COOLDOWN":
                    if now_ts >= sm_cooldown_until:
                        if sm_queue:
                            ...
                            if state_debug_log:
                                print(f"[SM] t={now_ts:.3f} COOLDOWN->SCAN rescan")
                ...
                if show and sm_target is not None:
                    ...
                    cv2.putText(
                        frame,
                        f"weed {best_conf:.2f} state={sm_state} q={len(sm_queue)}",
                        (int(x1), int(y1) - 8),
                        cv2.FONT_HERSHEY_SIMPLEX,
                        0.5,
                        (0, 255, 0),
